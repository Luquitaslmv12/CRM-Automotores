...
export default function CardClientesMorosos() {
  ...
  const exportarCSV = () => {
    ...
    const rows = morosos.map((c) => [c.nombre, c.telefono, c.email || "", c.etiqueta]);
  ...
  const exportarPDF = () => {
    ...
    doc.setFontSize(12);
    morosos.forEach((c, i) => {
      doc.text(`${i + 1}. ${c.nombre} - ${c.telefono} - ${c.email || "Sin email"}`, 20, 30 + i * 10);
    });
    doc.save("clientes_morosos.pdf");
  };

  return (
    <div className="relative bg-white dark:bg-gray-800 p-6 rounded-xl shadow border-l-4 border-red-500">
      <div className="flex items-center gap-4 mb-2">
        <Users className="text-red-600 w-8 h-8" />
        <h3 className="text-xl font-semibold flex-grow">Clientes Morosos</h3>
        <button
          onClick={exportarCSV}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 rounded mr-2 text-sm"
        >
          Exportar CSV
        </button>
        <button
        ...
        {morosos.slice(0, 3).map((c) => (
          <div key={c.id} className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <a href={`tel:${c.telefono}`} className="text-blue-600" title="Llamar">
                <Phone size={16} />
              </a>
              {c.email && (
                <a href={`mailto:${c.email}`} className="text-green-600" title="Email">
                  <Mail size={16} />
                </a>
              )}
              <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 p-6 rounded-xl max-w-sm w-full shadow-lg">
            ...
            <p className="text-sm mb-4">
              Estás por marcar a <strong>{clienteConfirmar.nombre}</strong> como cliente sin deuda.
              Esto significa que abonó la <strong>totalidad</strong> de la deuda y su estado cambiará a <em>Potencial</em>.
            </p>
